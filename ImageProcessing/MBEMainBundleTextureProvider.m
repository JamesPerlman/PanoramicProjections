...
- (void)setImage:(UIImage*)image context:(MBEContext*)context {
...
- (instancetype)initWithImageNamed:(NSString *)imageName context:(MBEContext *)context
{
    if ((self = [super init]))
    {
        [self setImage:[UIImage imageNamed:imageName] context:context];
    }
    return self;
}
...
- (id<MTLTexture>)textureForImage:(UIImage *)image context:(MBEContext *)context
{
    ...
    NSError *error = nil;
    id<MTLTexture>texture = [textureLoader newTextureWithCGImage:imageRef options:nil error:&error];
    
    if (error != nil) {
        NSLog(@"Something went wrong loading the texture: %@", [error localizedDescription]);
    }
    return texture;
    /*
